...
//	}
	...
	@GET
	public Response obterPessoasVinculadas() {
		List<Pessoa> listaPessoas = pessoaServico.listarPessoasVinculadas();
	...
	public Response deletarPessoa(@PathParam("id") Long id) {
		try {
			...
			System.out.println("teste negocio exception");
...
//	}
	...
	@GET
	@Path("{id}")
	public Response obterPessoaVinculada(@PathParam("id") Long id) {
		return pessoaServico.encontrarPessoaVinculada(id).map(pessoas -> Response.ok(pessoas).build()).orElseGet(() -> Response.status(Status.NOT_FOUND).build());
	}
